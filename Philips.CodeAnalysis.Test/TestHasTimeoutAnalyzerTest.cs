...
namespace Philips.CodeAnalysis.Test
{
	...
	public class TestHasTimeoutTest : AssertCodeFixVerifier
	{
		...
		{
			OtherClassSyntax = @"class TestTimeouts { public const int CiAppropriate = 1; public const int Integration = 2; public const int CiAcceptable = 3; public const int Smoke = 4; }; class TestDefinitions { public const string UnitTests = ""Unit""; public const string IntegrationTests = """"; public const string SmokeTests = """"; };";
		}
		...
		[DataTestMethod]
		[DataRow("[TestMethod, TestCategory(TestDefinitions.UnitTests), Timeout(TestTimeouts.Integration)]", 54, TestHasTimeoutAttributeAnalyzer.UnitTestTimeoutIncorrect)]
		[DataRow("[TestMethod, TestCategory(TestDefinitions.IntegrationTests), Timeout(TestTimeouts.CiAppropriate)]", 61, TestHasTimeoutAttributeAnalyzer.IntegrationTimeoutIncorrect)]
		[DataRow("[TestMethod, TestCategory(TestDefinitions.SmokeTests), Timeout(TestTimeouts.CiAppropriate)]", 55, TestHasTimeoutAttributeAnalyzer.SmokeTimeoutIncorrect)]
		public void TimeoutAttributeWrong(string methodAttributes, int errorOffset, string expectedError)
		{
			VerifyError(string.Empty, methodAttributes, 0, errorOffset, expectedError);
		}
...
";
		...
		protected override DiagnosticResult GetExpectedDiagnostic(int expectedLineNumberErrorOffset = 0, int expectedColumnErrorOffset = 0)
		{
			return new DiagnosticResult
			{
				...
				Locations = new[]
				{
					new DiagnosticResultLocation("Test0.cs", 12 + expectedLineNumberErrorOffset, 5 + expectedColumnErrorOffset)
				}
			};
		}
