...
- **File references**: Identifies `<license type="file">` elements and treats them as unknown licenses requiring review
...
When a package uses `<license type="file">` (referencing a license file rather than an SPDX expression), the analyzer cannot determine the actual license content and will flag it as an unknown license requiring manual review.
...
For packages with unacceptable or unknown licenses that you need to use, you can whitelist them by their package name in `Allowed.Licenses.txt`:

```text
# Allowed.Licenses.txt - Examples of package name whitelisting
# Standard license identifiers
MIT
Apache-2.0

# Whitelist specific packages by name (safer approach)
Newtonsoft.Json
Microsoft.EntityFrameworkCore
SomeCommercialPackage

# Package names are safer than license file names since they are unique identifiers
# This prevents accidental acceptance of other packages with the same license file name
```

**Important**: Whitelisting by package name is safer than whitelisting by license file names (like "LICENSE.md") because package names are unique identifiers. This prevents accidentally accepting dangerous packages that happen to use the same generic license file name.
