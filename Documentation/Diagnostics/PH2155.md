# PH2155: Avoid Packages with Unacceptable Licenses

| Property | Value  |
|--|--|
| Package | [Philips.CodeAnalysis.SecurityAnalyzers](https://www.nuget.org/packages/Philips.CodeAnalysis.SecurityAnalyzers) |
| Diagnostic ID | PH2155 |
| Category  | [Security](../Security.md) |
| Analyzer | [LicenseAnalyzer](https://github.com/philips-software/roslyn-analyzers/blob/main/Philips.CodeAnalysis.SecurityAnalyzers/LicenseAnalyzer.cs)
| CodeFix  | No |
| Severity | Info |
| Enabled By Default | No |

## Introduction

This rule identifies packages with unacceptable licenses that may pose compliance risks to your project.

> **⚠️ Important Disclaimer**: This analyzer is designed to shift-left the developer experience when adding new NuGet packages and should not be construed as a replacement for commercial products devoted to license compliance. Do not rely on this analyzer as a source of truth for license compliance decisions. The author is neither accountable nor responsible for any mistakes or omissions in license detection. This analyzer only examines NuGet package references and does not cover other ways to reference third-party modules such as direct assembly references, COM references, or other dependency mechanisms.

## Reason

Packages with unacceptable licenses, particularly copyleft licenses like GPL, can impose legal obligations on your project. These licenses may require you to open-source your entire project or comply with specific distribution terms. It's important to review and approve such dependencies before including them in your codebase.

## How to fix violations

1. **Review the package license**: Examine whether the license is truly incompatible with your project's requirements
2. **Find alternatives**: Look for packages with more permissive licenses (MIT, BSD, Apache-2.0)
3. **Add to allowed list**: If the license is acceptable for your use case, add it to the `Allowed.Licenses.txt` configuration file
4. **Remove the package**: If no suitable alternative exists and the license is unacceptable, remove the package dependency

## Examples

### Package with unacceptable license

```xml
<!-- This package reference might trigger PH2155 if it has a GPL license -->
<PackageReference Include="SomeGplPackage" Version="1.0.0" />
```

### Package with file-based license

```xml
<!-- This package reference will trigger PH2155 because type="file" licenses cannot be automatically verified -->
<PackageReference Include="SomePackageWithFileLicense" Version="1.0.0" />
```

In the package's `.nuspec` file:
```xml
<license type="file">LICENSE.md</license>
<licenseUrl>https://aka.ms/deprecateLicenseUrl</licenseUrl>
```

The analyzer will extract the file name (`LICENSE.md`) and require it to be whitelisted with the specific package name for security.

### Diagnostic message

```
Package 'SomeGplPackage' version '1.0.0' has an unacceptable license 'GPL-3.0'. Add 'SomeGplPackage GPL-3.0' to Allowed.Licenses.txt if this specific package and license combination is acceptable.
```

```
Package 'SomePackageWithFileLicense' version '1.0.0' has an unacceptable license 'LICENSE.md'. Add 'SomePackageWithFileLicense LICENSE.md' to Allowed.Licenses.txt if this specific package and license combination is acceptable.
```

### Resolution options

#### Option 1: Add to allowed licenses file

```xml
<!-- In your project file -->
<ItemGroup>
  <AdditionalFiles Include="Allowed.Licenses.txt" />
</ItemGroup>
```

```text
# Allowed.Licenses.txt - Combined format for maximum security
# Only accept specific packages with specific licenses
SomeGplPackage GPL-3.0
SomeCommercialPackage CommercialLicense-Vendor-X
SomePackageWithFileLicense LICENSE.md

# This prevents both license change issues and file name collisions
# If a package later changes to a different license, it will be flagged again
```

#### Option 2: Find alternative package

Replace the package with one that has a more permissive license:

```xml
<!-- Replace with alternative package -->
<PackageReference Include="AlternativePackage" Version="2.0.0" />
```

#### Option 3: Remove the dependency

If the package is not essential, remove it entirely:

```xml
<!-- Remove the problematic dependency -->
<!-- <PackageReference Include="SomeGplPackage" Version="1.0.0" /> -->
```

## How the Analyzer Works

The LicenseAnalyzer implements an offline package license checking system that analyzes your project's NuGet dependencies without requiring network access:

### Architecture

1. **Project Discovery**: Searches source file paths to locate your project's `project.assets.json` file (avoids using external dependencies for .NET Standard 2.0 compatibility)
2. **Package Analysis**: Parses the lock file using custom JSON parsing to extract all PackageReference dependencies  
3. **License Extraction**: Examines the local NuGet global packages cache to read `.nuspec` files and extract license information
4. **Performance Caching**: Builds and maintains a `licenses.cache` file in the temporary obj folder alongside `project.assets.json` to cache license information and avoid repeated file system operations
5. **Offline Operation**: Works entirely offline using your local NuGet cache without requiring network calls

### License Detection

The analyzer can detect licenses from multiple sources with enhanced extraction logic:

#### License Element Handling
- **SPDX expressions**: Reads modern `<license type="expression">MIT</license>` elements from .nuspec files
- **File references**: Extracts the actual file name from `<license type="file">LICENSE.md</license>` elements for specific whitelisting
- **License URLs**: Parses older `<licenseUrl>` elements and extracts license types from common URL patterns  
- **Fallback identification**: Recognizes common licenses like MIT, Apache-2.0, BSD, GPL, LGPL from URLs

#### Combined Entry Creation
The analyzer uses different license content for combined entries based on the package's license format:

1. **No license element**: Uses the `licenseUrl` content
   - Example: `<licenseUrl>https://aka.ms/deprecateLicenseUrl</licenseUrl>` → `PackageName aka.ms/deprecateLicenseUrl`

2. **License type="file"**: Uses the file name
   - Example: `<license type="file">LICENSE.md</license>` → `PackageName LICENSE.md`

3. **License type="expression"**: Uses the license expression  
   - Example: `<license type="expression">PostgreSQL</license>` → `PackageName PostgreSQL`

This approach provides meaningful license identification while maintaining the security benefits of combined package+license verification.

### Default Acceptable Licenses

The analyzer includes these permissive licenses as acceptable by default:
- MIT
- Apache-2.0  
- BSD-2-Clause
- BSD-3-Clause
- ISC
- Unlicense
- 0BSD
- PostgreSQL
- github.com/dotnet/corefx/blob/master/LICENSE.TXT
- github.com/dotnet/standard/blob/master/LICENSE.TXT
- go.microsoft.com/fwlink/?LinkId=329770
- www.bouncycastle.org/csharp/licence.html

## Configuration

This analyzer is **enabled by default**. You can disable it in your project file if needed:

```xml
<!-- Disable the analyzer if needed -->
<PropertyGroup>
  <WarningsNotAsErrors />
</PropertyGroup>
```

Or disable it via .editorconfig:

```ini
# Disable PH2155 analyzer
dotnet_diagnostic.PH2155.severity = none
```

Configure allowed licenses by creating an `Allowed.Licenses.txt` file in your project and adding it as an additional file:

```xml
<ItemGroup>
  <AdditionalFiles Include="Allowed.Licenses.txt" />
</ItemGroup>
```

The `Allowed.Licenses.txt` file supports:
- Combined package + license format: `packagename license`
- Comments using `#` at the beginning of lines
- Custom license names for commercial or proprietary packages
- SPDX license identifiers
- Entries are case-insensitive

### Whitelisting Specific Packages

For packages with unacceptable or unknown licenses that you need to use, use the combined package + license format in `Allowed.Licenses.txt`:

#### Built-in Acceptable Licenses (Automatic)

Packages with the following licenses are automatically accepted without needing any configuration:

- MIT
- Apache-2.0  
- BSD-2-Clause
- BSD-3-Clause
- ISC
- Unlicense
- 0BSD
- PostgreSQL
- github.com/dotnet/corefx/blob/master/LICENSE.TXT
- github.com/dotnet/standard/blob/master/LICENSE.TXT
- go.microsoft.com/fwlink/?LinkId=329770
- www.bouncycastle.org/csharp/licence.html

#### Combined Package + License Format (For Additional Packages)

For packages that are not automatically accepted, use the secure combined format: `packagename license`. This provides both package identity verification and license verification, preventing both license change vulnerabilities and license file name collisions.

```text
# Allowed.Licenses.txt - Combined format examples
# Combined package + license entries for packages not automatically accepted
Newtonsoft.Json MIT
Microsoft.EntityFrameworkCore Apache-2.0
SomeCommercialPackage CommercialLicense-2024
OldLegacyPackage LICENSE.txt

# Examples for different license sources:
# For type="expression": PackageName LicenseExpression
# For type="file": PackageName FileName  
# For licenseUrl only: PackageName NormalizedUrl

# This format prevents both vulnerabilities:
# - Package license changes (package must have specific license)
# - License file name collisions (entry is specific to one package)
```

### Security Benefits

The analyzer provides a two-tier security approach:
- **Built-in licenses**: Common permissive licenses are automatically accepted
- **Combined format**: For additional packages, the highest security level is required:
  - **Prevents license changes**: If a whitelisted package changes license, it will be flagged again
  - **Prevents file name collisions**: Generic names like "LICENSE.md" can't accidentally whitelist multiple packages
  - **Unique identification**: Each entry is specific to one package with one license

### Debug Logging Configuration

The analyzer provides detailed diagnostic logging to help troubleshoot issues with project.assets.json file discovery and license analysis. This is especially useful during development and deployment.

You can control debug logging via `.editorconfig`:

```ini
# Enable debug logging (default: enabled for now since analyzer is not production ready)
dotnet_code_quality.PH2155.enable_debug_logging = true

# Disable debug logging for production use
dotnet_code_quality.PH2155.enable_debug_logging = false
```

When enabled, the analyzer reports informational diagnostics about:
- Project.assets.json file search paths and results
- Source directory discovery from compilation
- File system operations and any exceptions
- Cache operations and license extraction results
- Configuration status and search completion

**Note**: Debug logging is currently enabled by default since this analyzer is disabled by default. When you enable the analyzer, you may want to disable debug logging for production use.

## Performance

The analyzer includes several performance optimizations:

### License Caching
- Creates a `licenses.cache` file in your project directory to cache license information
- Avoids repeated parsing of `.nuspec` files for the same packages
- Cache persists across builds for improved performance

### Offline Operation
- No network calls required - works entirely with local NuGet cache
- Leverages the existing `~/.nuget/packages` global packages folder
- Uses `project.assets.json` for accurate dependency information

### Graceful Degradation
- Handles missing dependencies (like NuGet.ProjectModel) gracefully in analyzer environments
- Skips analysis if `project.assets.json` cannot be located
- Continues operation even if individual package licenses cannot be determined

## Limitations

- Requires `project.assets.json` to be present (generated after `dotnet restore`)
- Depends on packages being available in the local NuGet global packages cache
- Some license detection relies on parsing `.nuspec` files and may not catch all license formats
- Currently disabled by default and must be explicitly enabled

